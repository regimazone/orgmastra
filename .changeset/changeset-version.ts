...
function format_readme_content(pr, short_sha, sha, _content) {
    return `- [${pr}](https://github.com/mastra-ai/mastra/pull/${pr.replace(
        "#",
        ""
    )}) [\`${short_sha}\`](https://github.com/mastra-ai/mastra/commit/${sha}) - ${_content.trim()}`;
}

const { packages } = await getPackages(process.cwd());

const packages_to_write: { name: string; dir: string; version: string; packge_json: any; changelog: string }[] = [];

for (const pkg of packages) {
    if (`"${pkg.packageJson.name}"` in changedPackages) {
        const current_version = pkg.packageJson.version;
        const bump = process.env.FINAL_TAG || changedPackages[`"${pkg.packageJson.name}"`].version;
        ...
        let deps_updated: [string, string][] = [];
