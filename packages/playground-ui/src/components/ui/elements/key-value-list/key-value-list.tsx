...
type Value = React.ReactNode | ValueItem[];
type KeyValueListProps = {
  data: { key: string; label: string; value: Value; icon?: React.ReactNode; separator?: React.ReactNode }[];
  LinkComponent: React.ComponentType;
  className?: string;
};

export function KeyValueList({ data, LinkComponent, className }: KeyValueListProps) {
  const { Link } = useLinkComponent();

  return (
    <dl className={cn('grid grid-cols-[auto_1fr] gap-x-[1rem] items-start', className)}>
      {data.map(({ label, value, icon, separator }) => {
        ...
        return (
          <>
            ...
            <dd
              className={cn(
                'flex flex-wrap gap-[.5rem] py-[0.25rem] min-h-[2.25rem] text-[0.875rem] items-center text-icon5 ',
