...
describe('KeywordCoverageMetric', () => {
  ...
  it('should ignore common words and stop words', async () => {
    const result = await scorer.evaluate(createTestRun('The quick brown fox', 'A quick brown fox'));
  ...
  it('should ignore numbers by default', async () => {
    const result = await scorer.evaluate(createTestRun('The 123 quick 456 brown fox', 'The quick brown fox'));
    expect(result.score).toBe(1);
  });
  ...
  it('should include coverage details in result', async () => {
    const result = await scorer.evaluate(createTestRun('quick brown fox', 'quick brown fox'));
