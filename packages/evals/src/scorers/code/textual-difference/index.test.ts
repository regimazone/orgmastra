...
describe('TextualDifferenceMetric', () => {
  ...
  it('should handle small differences', async () => {
    const result = await scorer.evaluate(createTestRun('The quick brown fox', 'The quick brown cat'));
    expect(result.score).toBeGreaterThan(0.8);
    expect(result.analyzeStepResult?.changes).toBe(1);
  });

  it('should handle word additions', async () => {
    const result = await scorer.evaluate(createTestRun('The quick brown fox', 'The very quick brown fox'));
    expect(result.score).toBeGreaterThan(0.7);
    expect(result.analyzeStepResult?.changes).toBe(1);
  });

  it('should handle word deletions', async () => {
    const result = await scorer.evaluate(createTestRun('The quick brown fox jumps', 'The quick fox jumps'));
    expect(result.score).toBeGreaterThan(0.7);
    expect(result.analyzeStepResult?.changes).toBe(1);
  });
