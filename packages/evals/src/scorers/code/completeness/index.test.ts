...
describe('CompletenessMetric', () => {
  ...
  describe('basic functionality', () => {
    it('should return high score for identical text', async () => {
      ...
      const result = await scorer.evaluate(run);
    ...
    it('should return lower score for simplified text missing elements', async () => {
      ...
      const result = await scorer.evaluate(createTestRun(original, simplified));
  ...
  describe('edge cases', () => {
    ...
    it('should handle empty original string', async () => {
      const result = await scorer.evaluate(createTestRun('', 'some text'));
    ...
    it('should handle null and undefined inputs', async () => {
      ...
      await expect(scorer.evaluate(createTestRun(null, ''))).rejects.toThrow();
      // @ts-expect-error Testing invalid input
      await expect(scorer.evaluate(createTestRun('', undefined))).rejects.toThrow();
    });
  });

  describe('special cases', () => {
    it('should handle lists and enumerations', async () => {
      const result = await scorer.evaluate(createTestRun('apples, oranges, and bananas', 'apples and bananas'));
      expect(result.score).toBeLessThan(0.8);
      expect(result.extractStepResult?.missingElements).toContain('oranges');
    });

    it('should handle repeated elements', async () => {
      const result = await scorer.evaluate(createTestRun('cat cat cat cats', 'cat cats'));
