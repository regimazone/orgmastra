...
import { KeywordCoverageScorer } from './index';
...
describe('KeywordCoverageScorer', () => {
  ...
  it('should ignore numbers by default', async () => {
    const result = await scorer.score({ input: 'The 123 quick 456 brown fox', output: 'The quick brown fox' });
    expect(result.score).toBe(1);
  });
  ...
  it('should include coverage details in result', async () => {
    const result = await scorer.score({ input: 'quick brown fox', output: 'quick brown fox' });
