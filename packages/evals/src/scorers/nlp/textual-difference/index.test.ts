...
import { TextualDifferenceScorer } from './index';

describe('TextualDifferenceScorer', () => {
  const scorer = new TextualDifferenceScorer();
  ...
  it('should handle small differences', async () => {
    const result = await scorer.score({ input: 'The quick brown fox', output: 'The quick brown cat' });
    expect(result.score).toBeGreaterThan(0.8);
    expect(result.info?.changes).toBe(1);
  });

  it('should handle word additions', async () => {
    const result = await scorer.score({ input: 'The quick brown fox', output: 'The very quick brown fox' });
    expect(result.score).toBeGreaterThan(0.7);
    expect(result.info?.changes).toBe(1);
  });

  it('should handle word deletions', async () => {
    const result = await scorer.score({ input: 'The quick brown fox jumps', output: 'The quick fox jumps' });
    expect(result.score).toBeGreaterThan(0.7);
    expect(result.info?.changes).toBe(1);
  });
