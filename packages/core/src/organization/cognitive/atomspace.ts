...
export class AtomSpace extends MastraBase {
  ...
  #config: Required<Omit<AtomSpaceConfig, 'storage' | 'vector'>> & 
    Pick<AtomSpaceConfig, 'storage' | 'vector'>;
  ...
  public getAtomsByType(type: AtomType): Atom[] {
    ...
    return Array.from(atomIds).map(id => this.#atoms.get(id)!).filter(Boolean);
  }
  ...
  public async queryAtoms(pattern: {
    ...
    if (pattern.outgoing) {
      candidates = candidates.filter(atom => 
        pattern.outgoing!.every(id => atom.outgoing?.includes(id))
      );
    }
    ...
    if (pattern.truthValueMin) {
      candidates = candidates.filter(atom => {
        ...
        return (!min.strength || tv.strength >= min.strength) &&
  ...
  async #vectorizeAtom(atom: Atom): Promise<void> {
    ...
    try {
      ...
      await this.#vector.upsert({
        ...
        vectors: [{
          id: atom.id,
          values: [0.1, 0.2, 0.3], // Placeholder values - would be generated by embedder
          metadata: {
            type: atom.type,
            name: atom.name,
