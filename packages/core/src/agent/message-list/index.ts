...
export class MessageList {
  ...
  private mastraMessageV2ToMastraMessageV3(v2Msg: MastraMessageV2): MastraMessageV3 {
    ...
    // Preserve the original content.content value in metadata if it exists
    // AND if it differs from what would be generated by concatenating text parts
    ...
    if (v2Msg.content.content !== undefined) {
      ...
      const concatenatedText = textParts.map(p => (p as any).text).join('');

      // Only preserve if content.content differs from concatenated text parts
      if (v2Msg.content.content !== concatenatedText) {
        v3Msg.content.metadata = {
          ...(v3Msg.content.metadata || {}),
          __originalContentString: v2Msg.content.content,
        };
      }
    }
  ...
  private static mastraMessageV3ToV2(v3Msg: MastraMessageV3): MastraMessageV2 {
    ...
    const originalContentString = (v3Msg.content.metadata as any)?.__originalContentString;
    if (typeof originalContentString === 'string') {
      // Restore the original content.content value
      v2Msg.content.content = originalContentString;
      // Remove the internal metadata field from the returned message
      if (v2Msg.content.metadata) {
        const { __originalContentString, ...cleanMetadata } = v2Msg.content.metadata as any;
