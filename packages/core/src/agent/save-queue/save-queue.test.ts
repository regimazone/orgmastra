...
describe('SaveQueueManager', () => {
  ...
  it('batches saves with debounce', async () => {
    ...
    list.add.input(makeTestMessage('m1', 'thread-1', 'user', 'Hello'));
    manager.batchMessages(list, 'thread-1');
    list.add.input(makeTestMessage('m2', 'thread-1', 'user', 'Hello'));
    manager.batchMessages(list, 'thread-1');
  ...
  it('clearDebounce cancels pending debounce', async () => {
    ...
    list.add.input(makeTestMessage('m1', 'thread-6', 'user', 'Hello'));
  ...
  it('should serialize saves with a save queue under rapid step completion', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      list.add.input(makeTestMessage(`m${i}`, threadId, 'user', `message ${i}`));
  ...
  it('should flush buffered parts via drainUnsavedMessages before persisting', async () => {
    ...
    list.add.input(makeTestMessage('m1', threadId, 'user', 'Hello'));
    list.add.response(makeTestMessage('m2', threadId, 'assistant', 'Hi there!'));
    list.add.input(makeTestMessage('m3', threadId, 'user', 'How are you?'));
