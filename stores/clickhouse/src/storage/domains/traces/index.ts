...
export class TracesClickHouse extends TracesStorage {
  ...
  async getTraces({
    ...
    if (filters) {
      Object.entries(filters).forEach(([key, value]) => {
        conditions.push(`${key} = {var_col_${key}:String}`);
  ...
  async getTracesPaginated(args: StorageGetTracesArg): Promise<PaginationInfo & { traces: Trace[] }> {
    ...
    if (fromDate) {
      conditions.push(`createdAt >= {var_from_date:DateTime64(3)}`);
      queryArgs.var_from_date = fromDate.getTime() / 1000; // Convert to Unix timestamp
    }

    if (toDate) {
      conditions.push(`createdAt <= {var_to_date:DateTime64(3)}`);
      queryArgs.var_to_date = toDate.getTime() / 1000; // Convert to Unix timestamp
    }
  ...
  async batchTraceInsert({ records }: { records: Record<string, any>[] }): Promise<void> {
    try {
      ...
      throw new MastraError(
        {
          id: 'CLICKHOUSE_STORAGE_BATCH_TRACE_INSERT_FAILED',
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
