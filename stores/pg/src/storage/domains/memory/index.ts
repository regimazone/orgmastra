...
import { MemoryStorage, resolveMessageLimit, TABLE_MESSAGES, TABLE_RESOURCES, TABLE_THREADS } from "@mastra/core/storage";
import type { StorageGetMessagesArg, PaginationArgs, PaginationInfo, StorageResourceType, } from "@mastra/core/storage";
...
export class MemoryPG extends MemoryStorage {
    ...
    async getThreadById({ threadId }: { threadId: string }): Promise<StorageThreadType | null> {
        try {
            ...
            const thread = await this.client.oneOrNone<StorageThreadType>(
                `SELECT 
          id,
          "resourceId",
          title,
    ...
    async updateThread({
        ...
        try {
            const thread = await this.client.one<StorageThreadType>(
 ...
 */
        ...
        try {
            const perPage =
                perPageInput !== undefined
                    ? perPageInput
                    : resolveMessageLimit({ last: selectBy?.last, defaultLimit: 40 });
            const currentOffset = page * perPage;
    ...
    async getResourceById({ resourceId }: { resourceId: string }): Promise<StorageResourceType | null> {
        ...
        const result = await this.client.oneOrNone<StorageResourceType>(`SELECT * FROM ${tableName} WHERE id = $1`, [
            resourceId,
        ]);
