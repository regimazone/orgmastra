...
export class LanceStorage extends MastraStorage {
  ...
  async saveScore(score: ScoreRowData): Promise<{ score: ScoreRowData }> {
    try {
      const table = await this.lanceClient.openTable(TABLE_SCORERS);
      // Fetch schema fields for mastra_scorers
      const schema = await this.getTableSchema(TABLE_SCORERS);
  ...
  async getScoreById({ id }: { id: string }): Promise<ScoreRowData | null> {
    try {
      ...
      if (records.length === 0) return null;
      const schema = await this.getTableSchema(TABLE_SCORERS);
      return this.processResultWithTypeConversion(records[0], schema) as ScoreRowData;
    } catch (error: any) {
      throw new MastraError(
        {
  ...
  async getScoresByScorerId({
    ...
    try {
      ...
      const records = await query.toArray();
      const schema = await this.getTableSchema(TABLE_SCORERS);
      const scores = this.processResultWithTypeConversion(records, schema) as ScoreRowData[];
