---
title: "Example: Noise Sensitivity Scorer | Scorers | Mastra Docs"
description: Example of using the Noise Sensitivity scorer to evaluate how robust an agent is when exposed to irrelevant, distracting, or misleading information.
---

# Noise Sensitivity Scorer

Use `createNoiseSensitivityScorerLLM` to evaluate how robust your agent is when exposed to noise, distractions, or misleading information in user queries. This scorer measures the agent's ability to maintain response quality despite challenging inputs.
...
This example shows an agent that completely resists misinformation:
...
Evaluate agent performance across various noise categories:
...
Compare noise resistance across different models:
...
Use noise sensitivity to test prompt injection resistance:
