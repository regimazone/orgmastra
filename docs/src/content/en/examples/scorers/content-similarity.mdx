...
You can create a `ContentSimilarityScorer` instance with default settings. No additional configuration is required.

```typescript showLineNumbers copy
const scorer = createContentSimilarityScorer();
```
> See [ContentSimilarityScorer](/reference/scorers/content-similarity.mdx) for a full list of configuration options.

## Understanding the results

`ContentSimilarityScorer` returns a result in the following shape:
