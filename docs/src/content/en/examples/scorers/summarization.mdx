...
Use `createSummarizationScorer` to evaluate how well a response captures key information from the source while maintaining factual accuracy.
...
import { createSummarizationScorer } from "@mastra/evals/scorers/llm";

const scorer = createSummarizationScorer({ model: openai("gpt-4o-mini") });
...
A high score indicates that the summary captures all key details from the input without introducing errors. The `analyzeStepResult` field confirms full alignment and complete coverage.
...
The summary receives a moderate score due to missing key information from the input. The `analyzeStepResult` field confirms factual alignment but highlights gaps in content coverage.
...
You can create a `SummarizationScorer` instance by providing a model. No additional configuration is required.

```typescript showLineNumbers copy
const scorer = createSummarizationScorer({ model: openai("gpt-4o-mini") });
```

> See [SummarizationScorer](/reference/scorers/summarization.mdx) for a full list of configuration options.

## Understanding the results

`SummarizationScorer` returns a result in the following shape:
...
<GithubLink
  marginTop='mt-16'
  link="https://github.com/mastra-ai/mastra/blob/main/examples/basics/scorers/summarization"
/>
