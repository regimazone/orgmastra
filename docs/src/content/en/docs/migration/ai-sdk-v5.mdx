...
Mastra provides compatibility mode to help with migration:
...
export const mastra = new Mastra({
  ...
  // Auto-detect mode (use headers/query params)
  aiSdkCompat: 'auto',
  
  // Native v5 mode (default)
  aiSdkCompat: 'v5',
});
```
...
When using `aiSdkCompat: 'auto'`, clients can specify format:
...
The compatibility mode handles stream format conversion during the transition.
